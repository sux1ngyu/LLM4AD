...
def _run_solve_worker(eva: Any, m: int, n: int, costs: Any, row_cover: Any, out_q: "mp.Queue") -> None:
    try:
        if not callable(eva):
            raise ValueError("Evaluator received a non-callable solve().")
        out_q.put(eva(m, n, costs, row_cover))
...
def _solve_with_timeout(
    ...
    p = mp.Process(target=_run_solve_worker, args=(eva, m, n, costs, row_cover, out_q))
    p.start()
    ...
    def evaluate(
        ...
        *,
        ...
        for filename, ins_text in self._datasets.items():
            cases = self.load_data(ins_text) #each instance
            ...
            for idx in selected_indices:
                ...
                try:
                    scores_full[idx] = self.eval_func(
                        ...
                        n=j["n"],
                        costs=j["costs"],
                        row_cover=j["row_cover"],
                        selected_columns=solve_res["selected_columns"],
                    )
                except Exception as e:
                    scores_full[idx] = f"Exception: {e}"

            normed = self.norm_score({filename: (scores_full, None)})
            if filename not in normed:
                continue
            normed_scores_full, _ = normed[filename]
            per_case[filename] = []
            vals = [
